...
def reports():
    # No authentication check - make this page publicly accessible
    ...
    if request.method == 'POST':
        ...
        try:
            # Use session user if available, otherwise use 'guest'
            user = session.get('user', 'guest')
            preview = render_template_string(signature, user=user)
        except Exception:
            preview = signature
    
    return render_template('reports.html', signature=signature, preview=preview, user=session.get('user', 'guest'))
